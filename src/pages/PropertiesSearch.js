...
export default function PropertiesSearch(){
 ...
 let priceFilter = dataProperti.price!== 'Semua' ? JSON.parse(dataProperti.price) : ''


  if(dataProperti !== null && !loading) {
    filterProperty = properties.filter(item => item.hakMilikType === dataProperti.hakMilikType).filter(item => item.kota === dataProperti.city)
    filterProperty = dataProperti.type === "Semua" ? filterProperty : filterProperty.filter(item => item.propertyType === dataProperti.type)
    filterProperty = dataProperti.price === "Semua" ? filterProperty : filterProperty.filter(item => (item.price > priceFilter.min && item.price < priceFilter.max))
  }
